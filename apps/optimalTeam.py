...
player_list = espn_fantasy_pull(year = 2023, leagueid=1661951033)
team_dict = espn_team_pull(year = 2023, leagueid=1661951033)
...
def create_solution_table(fg_value, ft_value, three_point_value, rebs, asts, stls, blks,
    ...
    if inclusion_list is None:
        inclusion_list = []
    if fleague:
        fantasy_players = espn_fantasy_pull(year = 2023, leagueid=1661951033)
        if fantasy_team:
    ...
    if (calculation_clicks is None) or (number_players is None):
        ...
        sals = temp_table['2023-24'].replace('[\$,]', '', regex=True).astype(float).to_numpy()
        names = temp_table['no_accents'].to_numpy()
        optimal_team = calculate_optimal_team(slots=int(number_players), max_cost=2000,
        ...
        #         scores=scores, sals=sals, names=names)
        #     optimal_team_table = temp_table[temp_table['no_accents'].isin(optimal_team)]
        optimal_team_table = optimal_team_table.rename(columns={"2023-24": "salary"})
